...
import '../widgets/dua_share_image_preview.dart';

class DuaShareImageService {
  static const _temporaryMaxAge = Duration(days: 1);
  static const _temporaryFilePrefixes = <String>['dua_'];

  static Future<Uint8List> capturePng({
    required HadithShareData data,
    required HadithShareThemeData theme,
    ...
    final captureTheme = theme.copyWith(
  ...
  static Future<File> savePng({
    ...
    final sanitizedName = (fileName ?? 'dua_share_card')
        .trim()
        .replaceAll(RegExp(r'[^a-zA-Z0-9_-]+'), '_')
        .replaceAll(RegExp(r'_+'), '_')
        .replaceAll(RegExp(r'^_+|_+$'), '');
    final resolvedFileName =
        sanitizedName.isEmpty ? 'dua_share_card' : sanitizedName;
  ...
  static Future<void> _deleteOldTemporaryFiles(Directory directory) async {
    ...
    try {
      ...
      AppLogger.warning(
        'Failed to clean old dua share temporary files.',
        error: error,
  ...
  static ui.FlutterView _resolveFlutterView() {
    ...
    throw StateError(
      'Could not access a FlutterView for off-screen dua rendering.',
    );
  }
  ...
  static Future<Uint8List> _pngBytes(ui.Image image) async {
    ...
    if (byteData == null) {
      throw StateError('Could not generate the dua share image.');
    }
