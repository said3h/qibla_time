...
final surahLoadResultProvider = FutureProvider.family<SurahLoadResult, SurahSummary>(
  (ref, summary) async {
    final service = ref.watch(quranServiceProvider);
...
class QuranService {
  ...
  static const _alafasyAudioBaseUrl = 'https://everyayah.com/data/Alafasy_128kbps';
  static const _legacyAlafasyAudioBaseUrl = 'https://cdn.islamic.network/quran/audio/128/ar.alafasy';
  ...
  Future<SurahDetail> _fetchFromApi(
    ...
    final ayahs = List.generate(arabicAyahs.length, (i) {
      ...
      return SurahAyah(
        ...
        transliteration: i < translitAyahs.length ? _readOptionalString(translitAyahs[i], 'text') : '',
        translation: i < translationAyahs.length ? _readOptionalString(translationAyahs[i], 'text') : '',
        audioUrl: _alafasyAudioUrlFor(summary.number, i + 1),
        tajweedHtml: _matchingTajweedText(
          tajweedAyahs,
          index: i,
          numberInSurah: numberInSurah,
        ),
  ...
  List<Map<String, dynamic>> _decodeQuranComUthmaniAyahs(
    ...
    return rawVerses.map((rawVerse) {
      ...
      final text = verse[textKey]?.toString().trim();
      if (text == null || text.isEmpty) {
        throw FormatException(
          'Respuesta $context invalida: falta $textKey.',
        );
      }
  ...
  Future<List<Map<String, dynamic>>> _fetchTajweedAyahs(
    SurahSummary summary,
  ) async {
    try {
      final response = await http
          .get(
            Uri.parse(
              '$_quranComBaseUrl/quran/verses/uthmani_tajweed?chapter_number=${summary.number}',
            ),
          )
          .timeout(const Duration(seconds: _timeoutSeconds));
      ...
      final ayahs = _decodeQuranComUthmaniAyahs(
        response.bodyBytes,
        context: 'tajweed',
        textKey: 'text_uthmani_tajweed',
      );
      ...
      return ayahs.length == summary.ayahCount ? ayahs : const <Map<String, dynamic>>[];
    } catch (error, stackTrace) {
      AppLogger.warning(
        'Quran tajweed unavailable for surah ${summary.number}; using plain Uthmani text.',
        error: error,
        stackTrace: stackTrace,
      );
  ...
  String _matchingTajweedText(
    List<Map<String, dynamic>> tajweedAyahs, {
    required int index,
    required int numberInSurah,
  }) {
    if (index >= tajweedAyahs.length) return '';
    final tajweedAyah = tajweedAyahs[index];
    final tajweedNumberInSurah = _tryReadInt(tajweedAyah, 'numberInSurah');
    if (tajweedNumberInSurah != numberInSurah) return '';
    return _readOptionalString(tajweedAyah, 'text');
  }
    ...
    for (final rawAyah in rawAyahs) {
      ...
      ayahs.add(
        SurahAyah(
          ...
          tajweedHtml: _readOptionalString(ayahData, 'tajweedHtml'),
        ),
      );
    }
  ...
  bool _isValidOfflineDetail(SurahDetail detail) {
    ...
    return detail.ayahs.every(
      (ayah) => ayah.number > 0 && ayah.numberInSurah > 0 && ayah.numberInSurah <= detail.summary.ayahCount && ayah.arabic.trim().isNotEmpty && !_isLikelyCorruptQuranText(ayah.arabic) && (ayah.translation.trim().isEmpty || !_isLikelyCorruptQuranText(ayah.translation)),
    );
  }
  ...
  void _validateQuranDetail(
    ...
    for (final ayah in detail.ayahs) {
      if (ayah.numberInSurah <= 0 || ayah.numberInSurah > expectedAyahCount || ayah.arabic.trim().isEmpty || _isLikelyCorruptQuranText(ayah.arabic)) {
        throw FormatException(
      ...
      if (languageCode != 'ar' && (translation.isEmpty || _isLikelyCorruptQuranText(translation))) {
        throw FormatException(
  ...
  SurahDetail _localizedOfflineDetail(
    ...
    final normalizedLanguage = _normalizeLanguageCode(languageCode);
    if (normalizedLanguage != 'ar' && normalizedLanguage != 'de' && normalizedLanguage != 'it' && normalizedLanguage != 'pt' && normalizedLanguage != 'ru' && normalizedLanguage != 'tr') {
      return detail;
    }
  ...
  static String _normalizeLanguageCode(String languageCode) {
    final normalized = languageCode.trim().toLowerCase().replaceAll('-', '_').split('_').first;
